...
#define SIGNAL_GEN_UI_REFRESH_MS 100U
...
int main(void)
{
	...
	uint8_t saved_osc_stop_bit = OSC_RUN;
	uint8_t display_span = ECG_DISPLAY_SPAN_MIN;
	enum application_mode app_mode = mode_oscilloscope;
  ...
  systick_config();		//时钟初始化
	...
	while(1)
	{
		...
		if(((app_mode == mode_oscilloscope) && (tft_timer_value >= OSC_UI_REFRESH_MS)) ||
		   ((app_mode == mode_ecg) && (tft_timer_value >= ECG_UI_REFRESH_MS)) ||
		   ((app_mode == mode_signal_generator) && (tft_timer_value >= SIGNAL_GEN_UI_REFRESH_MS)))
		{
			set_key_bit_value(PAUSE_MS_TIMER);
			set_tft_bit_value(PAUSE_MS_TIMER);
			if(app_mode == mode_oscilloscope)
			{
		...
		{
			if(app_mode == mode_ecg)
			{
				ECG_KeyHandle(key_handle[key1].key_pin,key_handle[key1].key_state);
			}
			else if(app_mode == mode_signal_generator)
			{
				SignalGen_KeyHandle(key_handle[key1].key_pin,key_handle[key1].key_state);
			}
			else
			{
		...
		if(key_handle[key2].key_state != KEY_NoPress)
		{
			if(app_mode == mode_ecg)
			{
				ECG_KeyHandle(key_handle[key2].key_pin,key_handle[key2].key_state);
			}
			else if(app_mode == mode_signal_generator)
			{
				SignalGen_KeyHandle(key_handle[key2].key_pin,key_handle[key2].key_state);
			}
			else
			{
		...
		if(key_handle[key3].key_state != KEY_NoPress)
		{
			if(app_mode == mode_ecg)
			{
				ECG_KeyHandle(key_handle[key3].key_pin,key_handle[key3].key_state);
			}
			else if(app_mode == mode_signal_generator)
			{
				SignalGen_KeyHandle(key_handle[key3].key_pin,key_handle[key3].key_state);
			}
			else
			{
		...
		{
			if(key_handle[keyd].key_state == KeyLongPress)
			{
				if(app_mode == mode_oscilloscope)
				{
					saved_osc_stop_bit = get_osc_stop_bit();
				...
				{
					ECG_Stop();
					app_mode = mode_signal_generator;
					SignalGen_Start();
					SignalGen_StaticUI();
				}
				else
				{
					SignalGen_Stop();
					app_mode = mode_oscilloscope;
					Set_ADC_Channel(ADC_CHANNEL_3);
					clear_adc_value();
					Register_oscShowData();
					set_osc_stop_bit(saved_osc_stop_bit);
					TFT_Fill(0,0,160,128,BLACK);
					TFT_StaticUI();
					if(saved_osc_stop_bit == OSC_PAUSE)
					{
						osc_redraw_cached_wave();
					}
				}
				set_tft_timer_value(0);
			}
			...
			{
				ECG_KeyHandle(key_handle[keyd].key_pin,key_handle[keyd].key_state);
			}
			else if(app_mode == mode_signal_generator)
			{
				SignalGen_KeyHandle(key_handle[keyd].key_pin,key_handle[keyd].key_state);
			}
			else
			{
		...
		if(ec11_handle.ec11_direction != ec11_static)
		{
			...
			{
				SignalGen_Rotate((ec11_handle.ec11_direction == ec11_forward) ? 1 : -1);
			}
