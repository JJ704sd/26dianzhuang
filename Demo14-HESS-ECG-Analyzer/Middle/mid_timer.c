...
static volatile uint32_t freq_updated_ms;
static volatile uint32_t first_capture_ms;
static volatile uint8_t freq_valid;
...
void TIMER2_IRQHandler(void)
{
	...
	if(capture_pending != 0U)
  {
		if(0 == ccnumber){
			...
			first_capture_ms = runtime_ms;
			ccnumber = 1;
		}
		else if(1 == ccnumber)
		{
			if (timer_measurement_is_fresh(runtime_ms, first_capture_ms,
			                               FREQ_SIGNAL_TIMEOUT_MS, 1U) == 0U)
			{
				readvalue1 = captured_value;
				timer2_overflow_count = 0U;
				first_capture_ms = runtime_ms;
			...
			freq_updated_ms = runtime_ms;
			freq_valid = (freq != 0U) ? 1U : 0U;
...
{
	uint32_t value = freq;
	uint32_t updated = freq_updated_ms;
	uint8_t valid = freq_valid;

	if (timer_measurement_is_fresh(runtime_ms, updated,
	                               FREQ_SIGNAL_TIMEOUT_MS, valid) == 0U)
	{
		return 0U;
	}
	return value;
