...
{   
    ...
    nvic_irq_enable(TIMER15_IRQn,0U);	//保持1ms采样节拍不被输入捕获长时间打断
...
void mx_tim2_init(void)
{
    ...
    gpio_mode_set(GPIOA, GPIO_MODE_AF, GPIO_PUPD_PULLDOWN, GPIO_PIN_6);	//下拉保证断线时输入稳定为低
    ...
    timer_icinitpara.icfilter    = FREQ_CAPTURE_FILTER;             //抑制短毛刺，保留100-2000Hz输入
    timer_input_capture_config(TIMER2,TIMER_CH_0,&timer_icinitpara);
    
    timer_auto_reload_shadow_enable(TIMER2);	//使能自动重装载值
    
    timer_interrupt_flag_clear(TIMER2,TIMER_INT_FLAG_CH0 | TIMER_INT_FLAG_UP);	//清除中断标志位
    
    timer_interrupt_enable(TIMER2,TIMER_INT_CH0 | TIMER_INT_UP);	//使能捕获及溢出中断
