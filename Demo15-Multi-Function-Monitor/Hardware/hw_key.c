...
void key_scanf(struct key_class *key_handle)
{
	...
	key_handle->key_state = KEY_NoPress;
	pressed = (uint8_t)(gpio_input_bit_get(key_handle->key_gpio,
										 key_handle->key_pin) == key_handle->key_press_level);
	if(pressed != 0U){
		if(key_handle->keyCnt < 120U){
			key_handle->keyCnt++;
		}

		/* Report one single-press event after 20ms debounce. */
		if((key_handle->keyCnt == 2U) && (key_handle->keyLongFlag == 0U)){
			key_handle->key_state = KeyPress;
			key_handle->keyLongFlag = 1U;
		}
	}else{
		key_handle->keyCnt = 0U;
		key_handle->keyLongFlag = 0U;
	}
}
