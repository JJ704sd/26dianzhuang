...
static void test_invalid_signal_clears_current_heart_rate(void)
{
    ecg_acq_core_t core;
    ecg_acq_result_t result = {0};
    uint16_t sample;
    ...
    for (sample = 0U; sample < (2U * ECG_ACQ_SAMPLE_RATE_HZ); ++sample)
    {
        result = ECGAcqCore_Process(&core, 2048U);
    }
    CHECK(result.quality == ECG_SIGNAL_LEAD_OFF);
...
static void test_display_sample_preserves_adc_amplitude(void)
{
    uint16_t i;
...
static void test_waveform_mapping_preserves_shape_and_margin(void)
{
    int8_t waveform_samples[200];
    ...
    CHECK(ECG_ACQ_SAMPLE_RATE_HZ == 250U);
    for (i = 0U; i < 200U; ++i)
    {
        waveform_samples[i] = (((i + 10U) % 20U) < 10U) ? 77 : -77;
    }
    fit_limited = ECGAcqCore_MapDisplaySamples(
        waveform_samples, 200U, 4U, 0U, 17, 109, 63, 44, plot, 156U);
...
int main(void)
{
    ...
    test_invalid_signal_clears_current_heart_rate();
    test_display_sample_preserves_adc_amplitude();
    test_waveform_mapping_preserves_shape_and_margin();
