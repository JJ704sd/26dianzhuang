...
void Key_Handle(volatile struct Oscilloscope *value)
{
	...
	{
		case KEY1PRESS:
            (*value).pwmOut = (uint16_t)((*value).pwmOut +
                              (((*value).timerPeriod * 4U) / 100U));
			...
			break;
        case KEY3PRESS:
            oldPeriod = (*value).timerPeriod;
            newPeriod = oldPeriod / 2U;
            if(newPeriod < 250U)
            {
                newPeriod = 1000U;
            }
            (*value).timerPeriod = newPeriod;
            (*value).outputFreq = (uint16_t)(1000000U / newPeriod);
            (*value).pwmOut = (uint16_t)((((*value).pwmOut * newPeriod) +
                                         (oldPeriod / 2U)) / oldPeriod);
            Set_Output_Freq(newPeriod);
            Set_Output_PWMComparex((*value).pwmOut);
			break;
        case KEY2PRESS:
            if((*value).outputEnabled == 0U)
            {
                (*value).outputEnabled=1U;
            ...
            break;
        case KEYDPRESS:
            (*value).paused = ((*value).paused == 0U) ? 1U : 0U;
            break;
        case KEYAPRESS:
