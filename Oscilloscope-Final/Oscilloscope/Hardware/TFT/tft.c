...
/* Font lookup uses two-byte GBK indexes; keep UI bytes independent of source encoding. */
...
{	
	...
	{
			sprintf(showData,"%3luKHz",(unsigned long)((*value).gatherFreq/1000U));
			TFT_ShowString(55,106,(uint8_t *)showData,BLACK,GREEN,16,0);  
			memset(showData,0,32);
	}
	else
	{
			sprintf(showData,"%3luHz ",(unsigned long)(*value).gatherFreq);
			TFT_ShowString(55,106,(uint8_t *)showData,BLACK,GREEN,16,0);  
			memset(showData,0,32);  
	}
	
	if((*value).outputEnabled == 1U)
	{   
