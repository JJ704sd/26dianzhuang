...
#define FAST_HISTORY_SAMPLES     1000U
#define FAST_DISPLAY_SAMPLES     400U
#define ECG_HISTORY_SAMPLES      1500U
#define ECG_MIN_RR_SAMPLES       75U
#define ECG_MAX_RR_SAMPLES       500U
#define ECG_REFRACTORY_SAMPLES   63U
...
static volatile int16_t ecg_history[ECG_HISTORY_SAMPLES];
...
static uint8_t app_mode = APP_MODE_SCOPE;
...
static int16_t previous_filtered;
static uint16_t signal_level = 32U;
static uint16_t noise_level = 4U;
static uint8_t qrs_active;
static uint16_t qrs_peak_value;
static uint32_t qrs_peak_sample;
static uint32_t qrs_start_sample;
static uint16_t rr_history[4];
static uint8_t rr_count;
static uint8_t rr_index;
...
static uint16_t ecg_threshold(void)
{
    uint16_t threshold;

    if(signal_level > noise_level){
        threshold = noise_level + (uint16_t)((signal_level - noise_level) * 2U / 5U);
    }else{
...
static void ecg_update_noise(uint16_t magnitude)
{
    if(magnitude > noise_level){
        noise_level += (uint16_t)((magnitude - noise_level + 15U) / 16U);
    }else{
        noise_level -= (uint16_t)((noise_level - magnitude) / 16U);
    }
}

static void ecg_accept_peak(uint16_t peak_value, uint32_t peak_sample)
{
    uint32_t rr_samples;
    uint32_t rr_sum = 0U;
    uint8_t i;

    signal_level = (uint16_t)((signal_level * 7U + peak_value) / 8U);

    if(ecg_peak_seen != 0U){
        rr_samples = peak_sample - ecg_last_peak_sample;
        if((rr_samples >= ECG_MIN_RR_SAMPLES) && (rr_samples <= ECG_MAX_RR_SAMPLES)){
            rr_history[rr_index] = (uint16_t)rr_samples;
            rr_index = (uint8_t)((rr_index + 1U) % 4U);
            if(rr_count < 4U){
                rr_count++;
            }
            for(i = 0U; i < rr_count; i++){
                rr_sum += rr_history[i];
            }
            rr_samples = rr_sum / rr_count;
            ecg_rr_ms = (uint16_t)((rr_samples * 1000U) / ECG_SAMPLE_RATE_HZ);
            ecg_bpm = (uint16_t)((60U * ECG_SAMPLE_RATE_HZ + (rr_samples / 2U)) / rr_samples);
        }else if(rr_samples > ECG_MAX_RR_SAMPLES){
            rr_count = 0U;
            rr_index = 0U;
            ecg_bpm = 0U;
            ecg_rr_ms = 0U;
    ...
    ecg_last_peak_sample = peak_sample;
}

static void ecg_detect_r_peak(int16_t sample)
{
    uint16_t magnitude = ecg_abs16(sample);
    uint16_t threshold = ecg_threshold();
    uint32_t since_last_peak = ecg_sample_count - ecg_last_peak_sample;

    if(qrs_active != 0U){
        if(magnitude > qrs_peak_value){
            qrs_peak_value = magnitude;
            qrs_peak_sample = ecg_sample_count;
        }

        if((magnitude < (threshold / 2U)) ||
           ((ecg_sample_count - qrs_start_sample) >= 30U)){
            if((qrs_peak_value >= threshold) &&
               ((ecg_peak_seen == 0U) ||
                ((qrs_peak_sample - ecg_last_peak_sample) >= ECG_REFRACTORY_SAMPLES))){
                ecg_accept_peak(qrs_peak_value, qrs_peak_sample);
            }else{
                ecg_update_noise(qrs_peak_value);
            }
            qrs_active = 0U;
            qrs_peak_value = 0U;
        }
    }else if((magnitude > threshold) &&
             ((ecg_peak_seen == 0U) || (since_last_peak >= ECG_REFRACTORY_SAMPLES))){
        qrs_active = 1U;
        qrs_peak_value = magnitude;
        qrs_peak_sample = ecg_sample_count;
        qrs_start_sample = ecg_sample_count;
    }else{
        ecg_update_noise(magnitude);
    }

    if((ecg_peak_seen != 0U) &&
       ((ecg_sample_count - ecg_last_peak_sample) > ECG_SIGNAL_TIMEOUT)){
        ecg_bpm = 0U;
        ecg_rr_ms = 0U;
        rr_count = 0U;
        rr_index = 0U;
...
static void ecg_process_sample(uint16_t raw_value)
{
    ...
    derivative = filtered_value - previous_filtered;
    previous_filtered = filtered_value;

    ecg_history[ecg_write_index] = filtered_value;
    ecg_write_index++;
    ...
    ecg_detect_r_peak((int16_t)(filtered_value + (derivative * 2)));
}
...
static void ecg_wave_show(uint16_t vref_value)
{
    ...
    ecg_vpp_mv = (uint16_t)(((uint32_t)max_abs * 2U * 1210U) / ecg_vref_value);
    if(ecg_vpp_mv > 9999U){
        ecg_vpp_mv = 9999U;
    }
...
static void scope_show_ui(void)
{
    ...
    uint32_t output_frequency = SignalOutput_GetValue();
    ...
    TFT_ShowString(2U, 0U,
                   (const uint8_t *)((scope_view_mode == SCOPE_VIEW_FREE) ?
                                     "FREE" :
                                     ((scope_view_mode == SCOPE_VIEW_RISING) ?
                                      "RISE" : "FALL")),
                   CYAN, BLACK, 16U, 0U);

    if(SignalOutput_IsEnabled() != 0U){
        TFT_ShowString(128U, 34U, (const uint8_t *)"ON ", GREEN, BLACK, 16U, 0U);
    }else{
        TFT_ShowString(128U, 34U, (const uint8_t *)"OFF", RED, BLACK, 16U, 0U);
    }

    TFT_ShowString(128U, 50U,
                   (const uint8_t *)SignalOutput_GetModeText(),
                   WHITE, BLACK, 16U, 0U);
...
{
    TFT_Fill(0U, 0U, 160U, 128U, BLACK);
    TFT_ShowString(2U, 0U, (const uint8_t *)"FREE", WHITE, BLACK, 16U, 0U);
    TFT_ShowString(80U, 0U, (const uint8_t *)"TB", WHITE, BLACK, 16U, 0U);
    ...
    TFT_ShowString(128U, 18U, (const uint8_t *)"OUT", WHITE, BLACK, 16U, 0U);
    TFT_ShowString(128U, 50U, (const uint8_t *)"SQU", WHITE, BLACK, 16U, 0U);
    TFT_ShowString(2U, 96U, (const uint8_t *)"Vpp", WHITE, BLACK, 16U, 0U);
...
void toggle_app_mode(void)
{
    app_mode = (app_mode == APP_MODE_SCOPE) ? APP_MODE_ECG : APP_MODE_SCOPE;
...
void key_scanf_handle(const uint16_t key_pin, const uint8_t key_state)
{
    if(key_pin == KEY1_Pin){
        if(key_state == KeyPress){
            SignalOutput_SetEnabled((SignalOutput_IsEnabled() == 0U) ? 1U : 0U);
        }
    }else if(key_pin == KEY2_Pin){
        if(key_state == KeyPress){
            SignalOutput_NextMode();
        }
    }else if(key_pin == KEY3_Pin){
        if(key_state == KeyPress){
            SignalOutput_Adjust(1);
        }
