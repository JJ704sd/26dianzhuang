...
#define VIEW_SWITCH_HOLD_MS  2000U
...
static volatile uint8_t view_switch_pending;
...
static void mode_switch_timer_callback(void)
{
    ...
    if(gpio_input_bit_get(KEY3_GPIO_Port, KEY3_Pin) == RESET){
        if(key3_hold_ms < VIEW_SWITCH_HOLD_MS){
            key3_hold_ms++;
        }
        if((key3_hold_ms >= VIEW_SWITCH_HOLD_MS) &&
           (key3_switch_latched == 0U)){
            view_switch_pending = 1U;
...
int main(void)
{
    ...
    while(1){
        ...
        if(mode_switch_pending != 0U){
            ...
            __enable_irq();
            toggle_app_mode();
            /* Undo the short KEY2 waveform-mode action from this long press. */
        ...
        if(range_switch_pending != 0U){
            ...
            __enable_irq();
            if(get_app_mode() == APP_MODE_SCOPE){
        ...
        if(view_switch_pending != 0U){
            __disable_irq();
            view_switch_pending = 0U;
            __enable_irq();
            if(get_app_mode() == APP_MODE_SCOPE){
                cycle_scope_view_mode();
                TFT_StaticUI();
            }
            /* Undo the short KEY3 parameter action from this long press. */
